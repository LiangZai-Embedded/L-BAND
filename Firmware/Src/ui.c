...
//	  sprintf(buf,"%.1fC",data->temperature);//XXC(temperature)
//	  u8g2_DrawStr(&u8g2,0,9,buf);
    ...
    u8g2_DrawStr(&u8g2,10,50,"I-RING");
    ...
    u8g2_DrawStr(&u8g2,20,120,buf);
	  ...
	  u8g2_SetFont(&u8g2,u8g2_font_open_iconic_human_2x_t);//batt icon
...
void app_stw(Data* data)
{
	...
	static FlagStatus cnt_flag;
  static uint8_t    delay_cnt;//interval between 2 vibration detection
	static uint8_t    cnt_mode;   //0:LAP  1:SPL

	static int8_t     spl_player;//max: 6 players
	static uint8_t    spl_min[6] = {0};
	static uint8_t    spl_sec[6] = {0};
	static uint8_t    spl_msec[6] = {0};
	...
	if(app_sta==DETECT_STA)
	{
		...
		if(VIB)
	  {
		 ...
		 if(cnt_mode==1&&cnt_flag==SET) //SPL
		 {
    ...
    for(uint8_t i=0;i<=5;i++)
		{
			if(spl_player>=i)
			{
				...
				u8g2_DrawStr(&u8g2,0,70+i*10,buf);
			}
		}
	}
  ...
  static app_sta_e  app_sta = DETECT_STA;
	...
	if(app_sta==DETECT_STA)
	{
	  ...
	  time_set[5] = data->min;
		...
		u8g2_SetFont(&u8g2,u8g2_font_sirclivethebold_tr);
    u8g2_DrawStr(&u8g2,0,30,"Please");
		u8g2_DrawStr(&u8g2,0,40,"shake!");
...
void app_info(Data* data)
{
  ...
  u8g2_DrawStr(&u8g2,0,30,"I-RING");
	u8g2_DrawStr(&u8g2,0,40,"      U1.0");
	
	u8g2_DrawStr(&u8g2,0,70,"GUI-");
	u8g2_DrawStr(&u8g2,0,80,"U8G2");
  
	u8g2_DrawStr(&u8g2,0,100,"MCU-");
	u8g2_DrawStr(&u8g2,0,110,"STM32");
	u8g2_DrawStr(&u8g2,0,120,"F030K6");
...
{	
  ...
  u8g2_DrawGlyph(&u8g2,12+3*x+280,90,71);	
	...
	u8g2_SetFont(&u8g2,u8g2_font_sirclivethebold_tr);
	u8g2_DrawStr(&u8g2,18+3*x+350,120,"INFO");
	u8g2_SetFont(&u8g2,u8g2_font_open_iconic_app_6x_t);
  u8g2_DrawGlyph(&u8g2,12+3*x+350,90,64);
